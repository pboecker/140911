...
})},ScrollSpy.prototype.process=function(){var i,scrollTop=this.$scrollElement.scrollTop()+this.options.offset,scrollHeight=this.getScrollHeight(),maxScroll=this.options.offset+scrollHeight-this.$scrollElement.height(),offsets=this.offsets,targets=this.targets,activeTarget=this.activeTarget;if(this.scrollHeight!=scrollHeight&&this.refresh(),scrollTop>=maxScroll)return activeTarget!=(i=targets[targets.length-1])&&this.activate(i);if(activeTarget&&scrollTop<=offsets[0])return activeTarget!=(i=targets[0])&&this.activate(i);for(i=offsets.length;i--;)activeTarget!=targets[i]&&scrollTop>=offsets[i]&&(!offsets[i+1]||scrollTop<=offsets[i+1])&&this.activate(targets[i])},ScrollSpy.prototype.activate=function(target){this.activeTarget=target,$(this.selector).parentsUntil(this.options.target,".active").removeClass("active");var selector=this.selector+'[data-target="'+target+'"],'+this.selector+'[href="'+target+'"]',active=$(selector).parents("li").addClass("active");active.parent(".dropdown-menu").length&&(active=active.closest("li.dropdown").addClass("active")),active.trigger("activate.bs.scrollspy")};var old=$.fn.scrollspy;$.fn.scrollspy=Plugin,$.fn.scrollspy.Constructor=ScrollSpy,$.fn.scrollspy.noConflict=function(){return $.fn.scrollspy=old,this},$(window).on("load.bs.scrollspy.data-api",function(){$('[data-spy="scroll"]').each(function(){var $spy=$(this);Plugin.call($spy,$spy.data())})})}(jQuery),+function($){"use strict";function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.tab");data||$this.data("bs.tab",data=new Tab(this)),"string"==typeof option&&data[option]()})}var Tab=function(element){this.element=$(element)};Tab.VERSION="3.2.0",Tab.prototype.show=function(){var $this=this.element,$ul=$this.closest("ul:not(.dropdown-menu)"),selector=$this.data("target");if(selector||(selector=$this.attr("href"),selector=selector&&selector.replace(/.*(?=#[^\s]*$)/,"")),!$this.parent("li").hasClass("active")){var previous=$ul.find(".active:last a")[0],e=$.Event("show.bs.tab",{relatedTarget:previous});if($this.trigger(e),!e.isDefaultPrevented()){var $target=$(selector);this.activate($this.closest("li"),$ul),this.activate($target,$target.parent(),function(){$this.trigger({type:"shown.bs.tab",relatedTarget:previous})})}}},Tab.prototype.activate=function(element,container,callback){function next(){$active.removeClass("active").find("> .dropdown-menu > .active").removeClass("active"),element.addClass("active"),transition?(element[0].offsetWidth,element.addClass("in")):element.removeClass("fade"),element.parent(".dropdown-menu")&&element.closest("li.dropdown").addClass("active"),callback&&callback()}var $active=container.find("> .active"),transition=callback&&$.support.transition&&$active.hasClass("fade");transition?$active.one("bsTransitionEnd",next).emulateTransitionEnd(150):next(),$active.removeClass("in")};var old=$.fn.tab;$.fn.tab=Plugin,$.fn.tab.Constructor=Tab,$.fn.tab.noConflict=function(){return $.fn.tab=old,this},$(document).on("click.bs.tab.data-api",'[data-toggle="tab"], [data-toggle="pill"]',function(e){e.preventDefault(),Plugin.call($(this),"show")})}(jQuery),+function($){"use strict";function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.affix"),options="object"==typeof option&&option;data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})}var Affix=function(element,options){this.options=$.extend({},Affix.DEFAULTS,options),this.$target=$(this.options.target).on("scroll.bs.affix.data-api",$.proxy(this.checkPosition,this)).on("click.bs.affix.data-api",$.proxy(this.checkPositionWithEventLoop,this)),this.$element=$(element),this.affixed=this.unpin=this.pinnedOffset=null,this.checkPosition()};Affix.VERSION="3.2.0",Affix.RESET="affix affix-top affix-bottom",Affix.DEFAULTS={offset:0,target:window},Affix.prototype.getPinnedOffset=function(){if(this.pinnedOffset)return this.pinnedOffset;this.$element.removeClass(Affix.RESET).addClass("affix");var scrollTop=this.$target.scrollTop(),position=this.$element.offset();return this.pinnedOffset=position.top-scrollTop},Affix.prototype.checkPositionWithEventLoop=function(){setTimeout($.proxy(this.checkPosition,this),1)},Affix.prototype.checkPosition=function(){if(this.$element.is(":visible")){var scrollHeight=$(document).height(),scrollTop=this.$target.scrollTop(),position=this.$element.offset(),offset=this.options.offset,offsetTop=offset.top,offsetBottom=offset.bottom;"object"!=typeof offset&&(offsetBottom=offsetTop=offset),"function"==typeof offsetTop&&(offsetTop=offset.top(this.$element)),"function"==typeof offsetBottom&&(offsetBottom=offset.bottom(this.$element));var affix=null!=this.unpin&&scrollTop+this.unpin<=position.top?!1:null!=offsetBottom&&position.top+this.$element.height()>=scrollHeight-offsetBottom?"bottom":null!=offsetTop&&offsetTop>=scrollTop?"top":!1;if(this.affixed!==affix){null!=this.unpin&&this.$element.css("top","");var affixType="affix"+(affix?"-"+affix:""),e=$.Event(affixType+".bs.affix");this.$element.trigger(e),e.isDefaultPrevented()||(this.affixed=affix,this.unpin="bottom"==affix?this.getPinnedOffset():null,this.$element.removeClass(Affix.RESET).addClass(affixType).trigger($.Event(affixType.replace("affix","affixed"))),"bottom"==affix&&this.$element.offset({top:scrollHeight-this.$element.height()-offsetBottom}))}}};var old=$.fn.affix;$.fn.affix=Plugin,$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},data.offsetBottom&&(data.offset.bottom=data.offsetBottom),data.offsetTop&&(data.offset.top=data.offsetTop),Plugin.call($spy,data)})})}(jQuery),function($,window,document,undefined){function AutoHidingNavbar(element,options){this.element=$(element),this.settings=$.extend({},defaults,options),this._defaults=defaults,this._name=pluginName,this.init()}function hide(autoHidingNavbar){_visible&&(autoHidingNavbar.element.addClass("navbar-hidden").animate({top:-autoHidingNavbar.element.height()},{queue:!1,duration:autoHidingNavbar.settings.animationDuration}),$(".dropdown.open .dropdown-toggle",autoHidingNavbar.element).dropdown("toggle"),_visible=!1)}function show(autoHidingNavbar){_visible||(autoHidingNavbar.element.removeClass("navbar-hidden").animate({top:0},{queue:!1,duration:autoHidingNavbar.settings.animationDuration}),_visible=!0)}function detectState(autoHidingNavbar){var scrollTop=$window.scrollTop(),scrollDelta=scrollTop-_previousScrollTop;if(_previousScrollTop=scrollTop,0>scrollDelta){if(_visible)return;(autoHidingNavbar.settings.showOnUpscroll||_hideOffset>=scrollTop)&&show(autoHidingNavbar)}else if(scrollDelta>0){if(!_visible)return void(autoHidingNavbar.settings.showOnBottom&&scrollTop+_windowHeight===$document.height()&&show(autoHidingNavbar));scrollTop>=_hideOffset&&hide(autoHidingNavbar)}}function scrollHandler(autoHidingNavbar){autoHidingNavbar.settings.disableAutohide||(_lastScrollHandlerRun=(new Date).getTime(),detectState(autoHidingNavbar))}function bindEvents(autoHidingNavbar){$document.on("scroll."+pluginName,function(){(new Date).getTime()-_lastScrollHandlerRun>_throttleDelay?scrollHandler(autoHidingNavbar):(clearTimeout(_scrollThrottleTimer),_scrollThrottleTimer=setTimeout(function(){scrollHandler(autoHidingNavbar)},_throttleDelay))}),$window.on("resize."+pluginName,function(){clearTimeout(_resizeThrottleTimer),_resizeThrottleTimer=setTimeout(function(){_windowHeight=$window.height()},_throttleDelay)})}function unbindEvents(){$document.off("."+pluginName),$window.off("."+pluginName)}var _hideOffset,pluginName="autoHidingNavbar",$window=$(window),$document=$(document),_scrollThrottleTimer=null,_resizeThrottleTimer=null,_throttleDelay=70,_lastScrollHandlerRun=0,_previousScrollTop=null,_windowHeight=$window.height(),_visible=!0,defaults={disableAutohide:!1,showOnUpscroll:!0,showOnBottom:!1,hideOffset:"auto",animationDuration:200};AutoHidingNavbar.prototype={init:function(){return this.elements={navbar:this.element},this.setDisableAutohide(this.settings.disableAutohide),this.setShowOnUpscroll(this.settings.showOnUpscroll),this.setShowOnBottom(this.settings.showOnBottom),this.setHideOffset(this.settings.hideOffset),this.setAnimationDuration(this.settings.animationDuration),_hideOffset="auto"===this.settings.hideOffset?this.element.height():this.settings.hideOffset,bindEvents(this),this.element},setDisableAutohide:function(value){return this.settings.disableAutohide=value,this.element},setShowOnUpscroll:function(value){return this.settings.showOnUpscroll=value,this.element},setShowOnBottom:function(value){return this.settings.showOnBottom=value,this.element},setHideOffset:function(value){return this.settings.hideOffset=value,this.element},setAnimationDuration:function(value){return this.settings.animationDuration=value,this.element},show:function(){return show(this),this.element},hide:function(){return hide(this),this.element},destroy:function(){return unbindEvents(this),show(this),$.data(this,"plugin_"+pluginName,null),this.element}},$.fn[pluginName]=function(options){var args=arguments;if(options===undefined||"object"==typeof options)return this.each(function(){$.data(this,"plugin_"+pluginName)||$.data(this,"plugin_"+pluginName,new AutoHidingNavbar(this,options))});if("string"==typeof options&&"_"!==options[0]&&"init"!==options){var returns;return this.each(function(){var instance=$.data(this,"plugin_"+pluginName);instance instanceof AutoHidingNavbar&&"function"==typeof instance[options]&&(returns=instance[options].apply(instance,Array.prototype.slice.call(args,1)))}),returns!==undefined?returns:this}}}(jQuery,window,document),function(){"use strict";var $,EkkoLightbox;$=jQuery,EkkoLightbox=function(element,options){var content,footer,header,_this=this;return this.options=$.extend({title:null,footer:null,remote:null},$.fn.ekkoLightbox.defaults,options||{}),this.$element=$(element),content="",this.modal_id=this.options.modal_id?this.options.modal_id:"ekkoLightbox-"+Math.floor(1e3*Math.random()+1),header='<div class="modal-header"'+(this.options.title||this.options.always_show_close?"":' style="display:none"')+'><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">'+(this.options.title||"&nbsp;")+"</h4></div>",footer='<div class="modal-footer"'+(this.options.footer?"":' style="display:none"')+">"+this.options.footer+"</div>",$(document.body).append('<div id="'+this.modal_id+'" class="ekko-lightbox modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content">'+header+'<div class="modal-body"><div class="ekko-lightbox-container"><div></div></div></div>'+footer+"</div></div></div>"),this.modal=$("#"+this.modal_id),this.modal_dialog=this.modal.find(".modal-dialog").first(),this.modal_content=this.modal.find(".modal-content").first(),this.modal_body=this.modal.find(".modal-body").first(),this.lightbox_container=this.modal_body.find(".ekko-lightbox-container").first(),this.lightbox_body=this.lightbox_container.find("> div:first-child").first(),this.showLoading(),this.modal_arrows=null,this.border={top:parseFloat(this.modal_dialog.css("border-top-width"))+parseFloat(this.modal_content.css("border-top-width"))+parseFloat(this.modal_body.css("border-top-width")),right:parseFloat(this.modal_dialog.css("border-right-width"))+parseFloat(this.modal_content.css("border-right-width"))+parseFloat(this.modal_body.css("border-right-width")),bottom:parseFloat(this.modal_dialog.css("border-bottom-width"))+parseFloat(this.modal_content.css("border-bottom-width"))+parseFloat(this.modal_body.css("border-bottom-width")),left:parseFloat(this.modal_dialog.css("border-left-width"))+parseFloat(this.modal_content.css("border-left-width"))+parseFloat(this.modal_body.css("border-left-width"))},this.padding={top:parseFloat(this.modal_dialog.css("padding-top"))+parseFloat(this.modal_content.css("padding-top"))+parseFloat(this.modal_body.css("padding-top")),right:parseFloat(this.modal_dialog.css("padding-right"))+parseFloat(this.modal_content.css("padding-right"))+parseFloat(this.modal_body.css("padding-right")),bottom:parseFloat(this.modal_dialog.css("padding-bottom"))+parseFloat(this.modal_content.css("padding-bottom"))+parseFloat(this.modal_body.css("padding-bottom")),left:parseFloat(this.modal_dialog.css("padding-left"))+parseFloat(this.modal_content.css("padding-left"))+parseFloat(this.modal_body.css("padding-left"))},this.modal.on("show.bs.modal",this.options.onShow.bind(this)).on("shown.bs.modal",function(){return _this.modal_shown(),_this.options.onShown.call(_this)}).on("hide.bs.modal",this.options.onHide.bind(this)).on("hidden.bs.modal",function(){return _this.gallery&&$(document).off("keydown.ekkoLightbox"),_this.modal.remove(),_this.options.onHidden.call(_this)}).modal("show",options),this.modal},EkkoLightbox.prototype={modal_shown:function(){var video_id,_this=this;return this.options.remote?(this.gallery=this.$element.data("gallery"),this.gallery&&(this.gallery_items="document.body"===this.options.gallery_parent_selector||""===this.options.gallery_parent_selector?$(document.body).find('*[data-toggle="lightbox"][data-gallery="'+this.gallery+'"]'):this.$element.parents(this.options.gallery_parent_selector).first().find('*[data-toggle="lightbox"][data-gallery="'+this.gallery+'"]'),this.gallery_index=this.gallery_items.index(this.$element),$(document).on("keydown.ekkoLightbox",this.navigate.bind(this)),this.options.directional_arrows&&this.gallery_items.length>1&&(this.lightbox_container.prepend('<div class="ekko-lightbox-nav-overlay"><a href="#" class="'+this.strip_stops(this.options.left_arrow_class)+'"></a><a href="#" class="'+this.strip_stops(this.options.right_arrow_class)+'"></a></div>'),this.modal_arrows=this.lightbox_container.find("div.ekko-lightbox-nav-overlay").first(),this.lightbox_container.find("a"+this.strip_spaces(this.options.left_arrow_class)).on("click",function(event){return event.preventDefault(),_this.navigate_left()}),this.lightbox_container.find("a"+this.strip_spaces(this.options.right_arrow_class)).on("click",function(event){return event.preventDefault(),_this.navigate_right()}))),this.options.type?"image"===this.options.type?this.preloadImage(this.options.remote,!0):"youtube"===this.options.type&&(video_id=this.getYoutubeId(this.options.remote))?this.showYoutubeVideo(video_id):"vimeo"===this.options.type?this.showVimeoVideo(this.options.remote):"instagram"===this.options.type?this.showInstagramVideo(this.options.remote):this.error('Could not detect remote target type. Force the type using data-type="image|youtube|vimeo"'):this.detectRemoteType(this.options.remote)):this.error("No remote target given")},strip_stops:function(str){return str.replace(/\./g,"")},strip_spaces:function(str){return str.replace(/\s/g,"")},isImage:function(str){return str.match(/(^data:image\/.*,)|(\.(jp(e|g|eg)|gif|png|bmp|webp|svg)((\?|#).*)?$)/i)},isSwf:function(str){return str.match(/\.(swf)((\?|#).*)?$/i)},getYoutubeId:function(str){var match;return match=str.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/),match&&11===match[2].length?match[2]:!1},getVimeoId:function(str){return str.indexOf("vimeo")>0?str:!1},getInstagramId:function(str){return str.indexOf("instagram")>0?str:!1},navigate:function(event){if(event=event||window.event,39===event.keyCode||37===event.keyCode){if(39===event.keyCode)return this.navigate_right();if(37===event.keyCode)return this.navigate_left()}},navigate_left:function(){var src;return this.showLoading(),1!==this.gallery_items.length?(0===this.gallery_index?this.gallery_index=this.gallery_items.length-1:this.gallery_index--,this.$element=$(this.gallery_items.get(this.gallery_index)),this.updateTitleAndFooter(),src=this.$element.attr("data-remote")||this.$element.attr("href"),this.detectRemoteType(src,this.$element.attr("data-type"))):void 0},navigate_right:function(){var next,src;return this.showLoading(),1!==this.gallery_items.length?(this.gallery_index===this.gallery_items.length-1?this.gallery_index=0:this.gallery_index++,this.$element=$(this.gallery_items.get(this.gallery_index)),src=this.$element.attr("data-remote")||this.$element.attr("href"),this.updateTitleAndFooter(),this.detectRemoteType(src,this.$element.attr("data-type")),this.gallery_index+1<this.gallery_items.length&&(next=$(this.gallery_items.get(this.gallery_index+1),!1),src=next.attr("data-remote")||next.attr("href"),"image"===next.attr("data-type")||this.isImage(src))?this.preloadImage(src,!1):void 0):void 0},detectRemoteType:function(src,type){var video_id;return"image"===type||this.isImage(src)?(this.options.type="image",this.preloadImage(src,!0)):"youtube"===type||(video_id=this.getYoutubeId(src))?(this.options.type="youtube",this.showYoutubeVideo(video_id)):"vimeo"===type||(video_id=this.getVimeoId(src))?(this.options.type="vimeo",this.showVimeoVideo(video_id)):"instagram"===type||(video_id=this.getInstagramId(src))?(this.options.type="instagram",this.showInstagramVideo(video_id)):this.error('Could not detect remote target type. Force the type using data-type="image|youtube|vimeo"')},updateTitleAndFooter:function(){var caption,footer,header,title;return header=this.modal_content.find(".modal-header"),footer=this.modal_content.find(".modal-footer"),title=this.$element.data("title")||"",caption=this.$element.data("footer")||"",title||this.options.always_show_close?header.css("display","").find(".modal-title").html(title||"&nbsp;"):header.css("display","none"),caption?footer.css("display","").html(caption):footer.css("display","none"),this},showLoading:function(){return this.lightbox_body.html('<div class="modal-loading">Loading..</div>'),this},showYoutubeVideo:function(id){var aspectRatio,height,width;return aspectRatio=560/315,width=this.$element.data("width")||560,width=this.checkDimensions(width),height=width/aspectRatio,this.resize(width),this.lightbox_body.html('<iframe width="'+width+'" height="'+height+'" src="//www.youtube.com/embed/'+id+'?badge=0&autoplay=1&html5=1" frameborder="0" allowfullscreen></iframe>'),this.modal_arrows?this.modal_arrows.css("display","none"):void 0},showVimeoVideo:function(id){var aspectRatio,height,width;return aspectRatio=500/281,width=this.$element.data("width")||560,width=this.checkDimensions(width),height=width/aspectRatio,this.resize(width),this.lightbox_body.html('<iframe width="'+width+'" height="'+height+'" src="'+id+'?autoplay=1" frameborder="0" allowfullscreen></iframe>'),this.modal_arrows?this.modal_arrows.css("display","none"):void 0},showInstagramVideo:function(id){var height,width;return width=this.$element.data("width")||612,width=this.checkDimensions(width),height=width,this.resize(width),this.lightbox_body.html('<iframe width="'+width+'" height="'+height+'" src="'+this.addTrailingSlash(id)+'embed/" frameborder="0" allowfullscreen></iframe>'),this.modal_arrows?this.modal_arrows.css("display","none"):void 0},error:function(message){return this.lightbox_body.html(message),this},preloadImage:function(src,onLoadShowImage){var img,_this=this;return img=new Image,(null==onLoadShowImage||onLoadShowImage===!0)&&(img.onload=function(){var image;return image=$("<img />"),image.attr("src",img.src),image.addClass("img-responsive"),_this.lightbox_body.html(image),_this.modal_arrows&&_this.modal_arrows.css("display","block"),_this.resize(img.width)},img.onerror=function(){return _this.error("Failed to load image: "+src)}),img.src=src,img},resize:function(width){var width_total;return width_total=width+this.border.left+this.padding.left+this.padding.right+this.border.right,this.modal_dialog.css("width","auto").css("max-width",width_total),this.lightbox_container.find("a").css("padding-top",function(){return $(this).parent().height()/2}),this},checkDimensions:function(width){var body_width,width_total;return width_total=width+this.border.left+this.padding.left+this.padding.right+this.border.right,body_width=document.body.clientWidth,width_total>body_width&&(width=this.modal_body.width()),width},close:function(){return this.modal.modal("hide")},addTrailingSlash:function(url){return"/"!==url.substr(-1)&&(url+="/"),url}},$.fn.ekkoLightbox=function(options){return this.each(function(){var $this;return $this=$(this),options=$.extend({remote:$this.attr("data-remote")||$this.attr("href"),gallery_parent_selector:$this.attr("data-parent"),type:$this.attr("data-type")},options,$this.data()),new EkkoLightbox(this,options),this})},$.fn.ekkoLightbox.defaults={gallery_parent_selector:"*:not(.row)",left_arrow_class:".glyphicon .glyphicon-chevron-left",right_arrow_class:".glyphicon .glyphicon-chevron-right",directional_arrows:!0,type:null,always_show_close:!0,onShow:function(){},onShown:function(){},onHide:function(){},onHidden:function(){}}}.call(this);